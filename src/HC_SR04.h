...
public:
  ...
  unsigned long getDuration_us(int slave = HC_SR04_MASTER) {
    return isSlaveValid(slave) ? _slaves[slave - 1]->getDuration_us() : _duration_us;
  }

  long getDist_cm(int slave = HC_SR04_MASTER) {
    return isSlaveValid(slave) ? _slaves[slave - 1]->getDist_cm() : (long)(_duration_us / 58.8235);
  }
  long getDist_mm(int slave = HC_SR04_MASTER) {
    return isSlaveValid(slave) ? _slaves[slave - 1]->getDist_mm() : (long)(_duration_us / 5.88235);
  }
  long getDist_inch(int slave = HC_SR04_MASTER) {
    return isSlaveValid(slave) ? _slaves[slave - 1]->getDist_inch() : (long)(_duration_us / 148.01864);
  }

  bool HasTrigger(int slave = HC_SR04_MASTER) {
    return isSlaveValid(slave) ? _slaves[slave - 1]->HasTrigger() : _trigger >= 0;
  }
  int getNumberOfSensors() {
    return _numSlaves + 1;
  }  // slaves + master sensors
  HCSR04_State getState(int slave = HC_SR04_MASTER) {
    return isSlaveValid(slave) ? _slaves[slave - 1]->getState() : _state;
  }
  bool isInterruptSupported(int slave = HC_SR04_MASTER) {
    return isSlaveValid(slave) ? _slaves[slave - 1]->isInterruptSupported() : _interruptSupported;
  }
...
private:
  ...
  void reset();
  void StartTrigger();
  bool isSlaveValid(int slave) {
    return (_slaves != NULL && slave > 0 && slave <= _numSlaves) ? true : false;
  }
...
public:
  HC_SR04()
    : HC_SR04_BASE(TECHO_PIN) {
    _inst = this;
  }

  HC_SR04(int trigger)
    : HC_SR04_BASE(TECHO_PIN, trigger) {
    _inst = this;
  }

  HC_SR04(int trigger, HC_SR04_BASE *slaves[], int numSlaves)
    : HC_SR04_BASE(TECHO_PIN, trigger, slaves, numSlaves) {
    _inst = this;
  }
